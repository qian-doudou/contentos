...
ContentOS 是面向本地生活短视频代运营团队的 AI 内容运营与项目管理平台。当前已完成第八阶段历史内容检索：支持 CSV / JSON 预览导入、可配置去重键、可失效 Content Embedding、同账号 Top10 召回、规则初筛和 `duplicate_judge` Top5 判定。没有 Embedding 或 LLM Key 时分别使用确定性中文 Fallback 和 Mock，本阶段不生成选题。
...
- Schema：`db/schema.ts`。
- Migration：`drizzle/`。
- Seed：`npm run db:seed`，幂等写入带 `is_demo` 标识的组织、5 位可切换成员、客户授权关系、德祥楼业务层级、2026 年 9 月计划、1 条结构化待办内容、3 条无经营指标的演示历史内容、6 条已确认档案 Memory、8 个系统 Skill 及版本快照，以及 2026 演示 AI 额度。`duplicate_judge` v2 启用本阶段结构化输入/输出合同；不预置虚构模型价格。
...
当前未接入真实 OAuth、抖音 API、脚本版本/客户审核对象、Shoot 任务、Publish 记录、自动发布、GMV、支付、视频生成、自动剪辑、数字人或企业生产数据。第八阶段不提供正式 AI Planner 或选题生成 API；检索能力作为独立服务和 Test Run 页供下一阶段使用。Context Builder 的 L4 仍保持同账号、结构化摘要和 10 条预算限制；Stage 09 再把候选生成与本阶段检索结果组合。真实百炼调用需配置有效 Key 并为实际模型名添加价格配置；没有价格时成本显示“未知”。
