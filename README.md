...
- `/scripts/new`：快捷写脚本。选账号和目标选项（默认参考月计划），可点选品牌已有产品；AI 生成 3 个真实候选，选择一条后保存选题并继续生成脚本。无需重填品牌定位、风格或结构化内容表单。
...
1. 工作台选择账号，点击 **开始写脚本**。
2. 保持“帮我推荐”，或选择曝光、信任、到店下单；可点选已有产品，补充想法可以留空。点击 **帮我想 3 个选题**。
3. 阅读真实生成的标题与开场，点选一条。高度重复或质量阻断的候选不可选，可点击 **换个角度** 后重新去重。
4. 点击 **就写这个脚本**，系统仅保存这一条并生成口播、分镜、产品植入和行动引导。
5. 成稿可以 **复制口播**、**修改脚本**（创建新版本）和提交客户审核。内容详情把脚本置于元数据前；分镜 JSON 等专业编辑选项默认折叠。

新入口完全复用现有 Planner / Context / Dedup / Script / Approval API，没有新业务表，也没有修改生产 Skill Prompt。`GET /api/ai/planner` 额外返回安全的 `scriptPointCost`，页面提前展示策划与脚本各自的积分。已有批量策划和手动内容编辑功能保留。
...
快捷写脚本改版验证（2026-09-11）：`typecheck`、`lint`、20 个测试文件 / 128 项测试及生产 `build` 通过。新增 6 项跨服务快捷流程测试，覆盖单选持久化、失败重试、保存响应丢失、旧候选失效及服务端权限。
