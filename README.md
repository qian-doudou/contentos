...
ContentOS 是面向本地生活短视频代运营团队的 AI 内容运营与项目管理平台。当前已完成第六阶段 AI 基础设施：在已有 Organization → Client → Brand → Store → Account → Monthly Plan → Content 业务链路上，加入可版本化 Skill、统一百炼千问客户端、Run Trace、Usage/成本、AI Points 与确定性 Mock。本阶段只提供基础设施和 Test Run，不生成正式内容策划。
...
- Schema：`db/schema.ts`。
- Migration：`drizzle/`。
- Seed：`npm run db:seed`，幂等写入带 `is_demo` 标识的组织、5 位可切换成员、客户授权关系、德祥楼业务层级、2026 年 9 月计划、1 条结构化内容、8 个系统 Skill 及其 v1 快照，以及 2026 演示 AI 额度。不预置虚构模型价格。
...
当前未接入真实 OAuth、抖音 API、脚本版本/客户审核对象、Shoot 任务、Publish 记录、自动发布、GMV、支付、视频生成、自动剪辑、数字人或企业生产数据。第六阶段不提供正式内容策划/脚本生成 API，Skill 页调用明确为 Test Run。真实百炼调用需配置有效 Key 并为实际模型名添加价格配置；没有价格时成本显示“未知”。
