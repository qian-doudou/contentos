import { z } from 'zod';
import { businessStatuses, contentGoals, contentPriorities, contentTypes, hookTypes } from '@/db/constants';
...
export const contentDefaults = {
  ...
  productText: '', ctaType: '', localElement: '', peopleJson: [], status: 'active' as const,
...
export const planQuerySchema = z.object({
  ...
  month: z.coerce.number().int().min(1).max(12).optional(), status: status.optional(),
...
export const contentQuerySchema = z.object({
  ...
  priority: z.enum(contentPriorities).optional(), operatorId: uuid.optional(), status: status.optional(),
...
export const planOptionSchema = z.object({ id: uuid, accountId: uuid, year: z.number().int(), month: z.number().int(), status });
