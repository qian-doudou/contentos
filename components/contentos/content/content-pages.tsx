...
import { contentGoals, contentPriorities, contentTypes } from '@/db/constants';
import {
  contentDetailSchema, contentGoalLabels, contentListSchema, contentTypeLabels, hookTypeLabels, priorityLabels,
} from '@/lib/content/contracts';
import { EmptyData, ErrorData, LoadingData, Tags, useApiData } from '@/components/contentos/master-data/common';
import { ActiveBadge, ContentHeading, ContentNav, formatLocalDate, periodLabel } from './common';
...
export function ContentListPage() {
  ...
  return <div className="space-y-6">
    ...
    {state.loading ? <LoadingData /> : state.error ? <ErrorData error={state.error} retry={state.reload} /> : data && <>
      <form className="surface-card grid items-end gap-3 sm:grid-cols-2 xl:grid-cols-[2fr_1.4fr_1fr_1fr_1fr_auto_auto]" key={query} onSubmit={filter}>
      ...
      <section className="surface-card !p-0">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b p-5"><h2 className="font-semibold">内容列表</h2><Badge variant="outline">共 {data.total} 条</Badge></div>
        {data.items.length ? <Table><TableHeader><TableRow><TableHead>内容</TableHead><TableHead>账号 / 月度</TableHead><TableHead>类型 / 目标</TableHead><TableHead>运营</TableHead><TableHead>发布 / 截止</TableHead><TableHead>优先级</TableHead><TableHead>状态</TableHead></TableRow></TableHeader><TableBody>{data.items.map(item => <TableRow key={item.id}>
          ...
          <TableCell><Badge variant="secondary" className={priorityTone[item.priority]}>{priorityLabels[item.priority]}</Badge></TableCell><TableCell><ActiveBadge status={item.status} demo={item.isDemo} /></TableCell>
...
export function ContentDetailPage({ id }: { id: string }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="surface-card"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex flex-wrap gap-2"><Badge>{contentTypeLabels[item.contentType]}</Badge><Badge variant="secondary">{contentGoalLabels[item.contentGoal]}</Badge><Badge variant="secondary" className={priorityTone[item.priority]}>{priorityLabels[item.priority]}</Badge></div><ActiveBadge status={item.status} demo={item.isDemo} /></div>
