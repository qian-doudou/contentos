...
export function ContentWorkflowPanel({ content, onChanged }: { content: Content; onChanged: () => void }) {
  ...
  return <section className="surface-card">
    ...
    {restricted.length > 0 && <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">后续可达状态 {restricted.map(rule => contentStatusLabels[rule.to]).join('、')} 需由审核、拍摄、剪辑或发布业务 API 在同一事务中触发，此处不可直接修改。</p>}
