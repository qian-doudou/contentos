...
function Stepper({ step }: { step: number }) {
  return <ol aria-label="写脚本进度" className="grid grid-cols-3 gap-3 rounded-2xl border bg-white p-4 sm:p-5">
    {['选账号和方向', '选一个AI选题', '拿到脚本'].map((label, index) => <li key={label} aria-current={step === index + 1 ? 'step' : undefined} className={`flex items-center gap-2 text-sm ${step >= index + 1 ? 'font-medium text-cyan-900' : 'text-slate-400'}`}><span className={`grid size-7 shrink-0 place-items-center rounded-full ${step >= index + 1 ? 'bg-cyan-100 text-cyan-800' : 'bg-slate-100'}`}>{step > index + 1 ? <Check className="size-4" /> : index + 1}</span>{label}</li>)}
  </ol>;
}
...
function AccountContext({ accountId, product, onProduct }: { accountId: string; product: string; onProduct: (value: string) => void }) {
  ...
  if (state.loading) return <output className="text-sm text-slate-500">正在读取品牌资料…</output>;
  ...
  return <div className="space-y-4">
    <div className="rounded-xl bg-cyan-50/70 p-4 text-sm leading-6"><p className="flex items-center gap-2 font-medium text-cyan-900"><CheckCircle2 className="size-4" />已带入 {brand.brandName} 的品牌资料</p><p className="mt-2 text-slate-600">{brand.brandPositioning || '定位尚未填写'}{account.contentStyleJson.length ? ` · ${account.contentStyleJson.join('、')}` : ''}</p><p className="mt-1 text-slate-500">有效品牌记忆与历史内容会自动参考，无需重新填写。</p></div>
    {brand.coreProductsJson.length > 0 && <fieldset><legend className="mb-2 text-sm font-medium">这次想重点讲什么？<span className="ml-2 font-normal text-slate-400">可直接跳过</span></legend><RadioGroup value={product} onValueChange={(value) => onProduct(String(value))} className="flex flex-wrap gap-2" aria-label="重点产品">
      {['', ...new Set(brand.coreProductsJson)].map((value) => <label key={value} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm ${product === value ? 'border-cyan-500 bg-cyan-50 text-cyan-900' : 'border-slate-200'}`}><RadioGroupItem value={value} />{value || 'AI帮我选'}</label>)}
    </RadioGroup></fieldset>}
  </div>;
}
...
export function ScriptWriterPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {notice && <output className="block rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">{notice}</output>}
    {pending && <output aria-live="polite" className="flex items-center gap-3 rounded-xl border border-cyan-200 bg-cyan-50 p-5"><LoaderCircle className="size-5 shrink-0 animate-spin text-cyan-700" /><span><span className="block font-medium text-cyan-950">{pending === 'topics' ? '正在为你想选题，并检查历史重复…' : pending === 'saving' ? '正在保存你选中的选题…' : pending === 'script' ? '正在写口播和分镜…' : pending === 'memory' ? '正在确认品牌资料…' : '正在换角度，并重新检查重复…'}</span><span className="mt-1 block text-sm text-cyan-800">这可能需要一点时间，请保持页面打开。</span></span></output>}
    {!session && !contentId && <section className="surface-card space-y-6 sm:!p-7">
      {!writableAccounts.length ? <EmptyData title="还没有可写脚本的账号" description="先添加一个品牌账号；已有账号请联系负责人分配客户。"><Button nativeButton={false} render={<Link href="/accounts" />}>查看品牌与账号</Button></EmptyData> : <>
        ...
        <fieldset disabled={Boolean(pending)}><legend className="mb-3 text-base font-medium">这条视频想达到什么效果？</legend><RadioGroup aria-label="视频目标" value={goal} onValueChange={(value) => setGoal(value as Goal | 'auto')} className="grid gap-3 sm:grid-cols-2">
          {goalOptions.map((option) => <label key={option.value} htmlFor={`goal-${option.value}`} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${goal === option.value ? 'border-cyan-500 bg-cyan-50/60 ring-1 ring-cyan-500' : 'hover:border-slate-400'}`}><RadioGroupItem id={`goal-${option.value}`} className="mt-1" value={option.value} /><span><span className="block font-medium">{option.label}</span><span className="mt-1 block text-sm text-slate-500">{option.value === 'auto' && activeAccount?.currentPlan ? `本月重点：${contentGoalLabels[activeAccount.currentPlan.primaryGoal]}` : option.description}</span></span></label>)}
        </RadioGroup></fieldset>
        ...
        <details className="rounded-xl border p-4"><summary className="cursor-pointer text-sm text-slate-600">还有特别想说的？（选填）</summary><Textarea aria-label="补充想法" value={extra} disabled={Boolean(pending)} onChange={(event) => setExtra(event.target.value)} maxLength={1500} className="mt-3" placeholder="例如：这次想讲老板为什么坚持手切。留空也可以。" /></details>
    ...
    {session && !contentId && <section className="space-y-4">
      ...
      <RadioGroup value={selected} onValueChange={(value) => setSelected(String(value))} aria-label="AI选题" className="grid items-start gap-4 lg:grid-cols-3">
        {session.candidates.map((candidate) => <article key={candidate.id} className={`surface-card flex h-full min-w-0 flex-col !p-0 ${selected === candidate.id ? '!border-cyan-500 ring-2 ring-cyan-100' : ''}`}>
          ...
          <div className="space-y-3 border-t p-4"><details className="text-sm"><summary className="cursor-pointer text-slate-600">为什么推荐 / 相似内容</summary><p className="mt-3 leading-6">{candidate.recommendedReason}</p><p className="mt-2 leading-6 text-slate-500">{candidate.duplicateReason}</p>{candidate.qualityIssues.map((issue) => <p key={`${issue.code}:${issue.field}`} className="mt-2 text-amber-800">{issue.message}</p>)}{candidate.similarContents.slice(0, 5).map((item) => <Link key={item.contentId} href={`/contents/${item.contentId}`} target="_blank" className="mt-2 block text-cyan-800 underline">{item.title}</Link>)}</details><Button variant="outline" className="h-10 w-full" disabled={Boolean(pending) || session.status !== 'awaiting_selection'} onClick={() => void reangle(candidate.id, candidate.alternativeAngles[0])}><RefreshCw />换个角度</Button></div>
        </article>)}
      </RadioGroup>
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-white p-5"><p className="text-sm text-slate-600">{selected ? '选题已选好。接下来自动写出口播、开场和分镜。' : '点选上面一个选题，再开始写脚本。'}<span className="mt-1 block text-slate-400">只保存你选中的这一条。{estimate !== null ? `本次完整生成共 ${estimate} 积分。` : ''}</span></p><Button className="h-12 px-6 text-base" disabled={!selected || Boolean(pending) || !canGenerate || session.status !== 'awaiting_selection'} onClick={() => void writeScript()}><Sparkles />就写这个脚本<ArrowRight /></Button></div>
    </section>}
