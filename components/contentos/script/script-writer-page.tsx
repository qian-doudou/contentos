...
export function ScriptWriterPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-7">
    ...
    {contentId && !pending && <><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="flex items-center gap-2 font-semibold text-emerald-950"><CheckCircle2 className="size-5" />脚本已保存到脚本库</p><p className="mt-1 text-sm leading-6 text-emerald-800">当前版本和后续 V2、客户审核记录都会保存在这条内容下。以后可从左侧“脚本库”再次找到。</p></div><div className="flex flex-wrap gap-2"><Button size="sm" nativeButton={false} render={<Link href="/scripts" />}>打开脚本库</Button><Button size="sm" variant="outline" nativeButton={false} render={<Link href={`/contents/${contentId}#scripts`} />}>查看内容详情</Button></div></div></div><div className="flex flex-wrap items-center justify-between gap-3"><Link className="text-sm text-cyan-800 underline" href={`/contents/${contentId}`}>查看完整内容与拍摄进度</Link><Button variant="outline" onClick={() => window.location.assign('/scripts/new')}>再写一条</Button></div><ScriptApprovalPanel key={`${contentId}:${revision}`} contentId={contentId} onContentChanged={() => {}} /></>}
  </div>;
}
