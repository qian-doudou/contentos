...
export function ScriptStartCard() {
  ...
  return <section className="grid gap-6 rounded-lg border border-[#e9e9e7] bg-white p-6 md:grid-cols-[1fr_minmax(250px,0.62fr)]" aria-labelledby="start-script-title">
    ...
    <div className="flex flex-col justify-center gap-3 rounded-md border border-[#e9e9e7] bg-[#f7f7f5] p-4">
      {state.loading ? <output className="text-sm text-[#787774]">正在读取可用账号…</output> : state.error ? <><p role="alert" className="text-sm text-rose-700">账号加载失败，请重试。</p><Button variant="outline" onClick={state.reload}>重新加载账号</Button></> : accounts.length ? <><label htmlFor="home-script-account" className="text-sm font-medium text-[#37352f]">今天给谁写？</label><NativeSelect id="home-script-account" className="h-10 w-full bg-white" value={selected?.id} onChange={(event) => setAccountId(event.target.value)}>{accounts.map((item) => <option value={item.id} key={item.id}>{item.clientName} · {item.accountName}</option>)}</NativeSelect><Button className="h-10 w-full" nativeButton={false} render={<Link href={`/scripts/new?accountId=${selected?.id}`} />}><Sparkles />开始写脚本<ArrowRight /></Button><Link href="/contents" className="text-center text-sm text-[#787774] underline underline-offset-4">已有选题？继续已有内容</Link></> : <><p className="text-sm leading-6 text-[#787774]">先添加品牌账号，或请负责人为你分配客户。</p><Button nativeButton={false} render={<Link href="/accounts" />}>添加 / 查看账号<ArrowRight /></Button></>}
    </div>
  </section>;
}
