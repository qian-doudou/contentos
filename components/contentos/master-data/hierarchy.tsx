  ...
  return <div className="space-y-5">{data.clients.map(client => {
    ...
    return <section className="surface-card space-y-5" key={client.id}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-4">
        ...
        {data.permissions.canWrite && <EditorDialog kind="brand" hierarchy={data} parent={{ clientId: client.id }} onSaved={onSaved} />}
      </div>
      ...
      {brands.map(brand => {
        ...
        return <details open className="rounded-xl border bg-slate-50/50 p-4" key={brand.id}>
          ...
          <div className="mt-4 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3"><div className="space-y-2"><BusinessBadge value={brand.status} demo={brand.isDemo} /><p className="text-sm text-slate-600">{brand.brandPositioning || '品牌定位未填写'}</p><Tags values={brand.coreProductsJson} /></div>{data.permissions.canWrite && <div className="flex flex-wrap gap-2"><EditorDialog kind="brand" initial={brand} hierarchy={data} onSaved={onSaved} /><EditorDialog kind="store" hierarchy={data} parent={{ brandId: brand.id, clientId: client.id }} onSaved={onSaved} /></div>}</div>
            ...
            {stores.map(store => {
              ...
              return <div className="overflow-hidden rounded-xl border bg-white" key={store.id}>
                <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
                  ...
                  {data.permissions.canWrite && <div className="flex flex-wrap gap-2"><EditorDialog kind="store" initial={store} hierarchy={data} onSaved={onSaved} /><EditorDialog kind="account" hierarchy={data} parent={{ clientId: client.id, brandId: brand.id, storeId: store.id }} onSaved={onSaved} /></div>}
                </div>
...
export function AccountsPage() {
  ...
  return <div className="space-y-6">
    <PageHeading title="品牌与账号" description="客户 → 品牌 → 门店 → 账号，沿业务归属查看全部资产。">
      {state.data?.permissions.canWrite && <Button variant="outline" nativeButton={false} render={<Link href="/clients/new" />}><Plus />新建客户</Button>}
      {state.data?.permissions.canWrite && <EditorDialog kind="account" hierarchy={state.data} onSaved={saved} />}
    </PageHeading>
