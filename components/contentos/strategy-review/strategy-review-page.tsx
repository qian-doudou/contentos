...
function ReviewPanel({ item, onChanged }: { item: StrategyReviewView; onChanged: (item: StrategyReviewView) => void }) {
  ...
  async function confirm() {
    ...
    try {
      ...
      setMessage(result.performanceMemoryId ? '策略与表现规律均已人工确认并写入 Memory。' : '策略已确认并写入长期 Memory。');
  ...
  async function reject() {
    ...
    try {
      ...
      onChanged(result); setMessage('已驳回；未写入任何 Memory。');
    } catch (error) { setMessage(errorText(error)); } finally { setPending(false); }
  }
  ...
  return <div className="space-y-6">
    ...
    {item.status === 'draft' && item.permissions.canConfirm && <section className="surface-card"><h3 className="font-semibold">人工确认门禁</h3><p className="mt-2 text-sm text-slate-500">AI 输出目前只是草稿，未进入 Active Memory。确认策略后才保存策略 Memory。</p><label htmlFor={`save-pattern-${item.id}`} className="mt-4 flex items-start gap-3 rounded-xl border p-3 text-sm"><Checkbox id={`save-pattern-${item.id}`} checked={savePattern} onCheckedChange={(value) => setSavePattern(value === true)} disabled={!item.sampleThresholdMet || pending} /><span><span className="font-medium">同时保存为表现规律</span><span className="mt-1 block text-slate-500">需达到管理员阈值 {item.sampleThreshold} 条；当前 {item.metricsSnapshotJson.sampledContentCount} 条。{!item.sampleThresholdMet && ' 当前不可选。'}</span></span></label><div className="mt-4 flex flex-wrap gap-2"><Button onClick={() => void confirm()} disabled={pending}>{pending ? <LoaderCircle className="animate-spin" /> : <CheckCircle2 />}确认策略</Button><Button variant="outline" onClick={() => void reject()} disabled={pending}><XCircle />驳回</Button></div></section>}
